...
`;
    ...
    if (!parsed || !Array.isArray(parsed.titles)) {
      ...
      const fallback = raw.split(/\r?\n/).map((s) => s.replace(/^[0-9\.\-\)\s]+/, "").trim()).filter(Boolean).slice(0,5);
...
JSON フィールド:
{
  ...
  "text": "プレーンテキスト（見出しは ###H2: タイトル のように分かりやすく）"
}
...
10) HTML と同じ内容のプレーンテキストも "text" に入れること（見出しは ### H2: 見出し のように分かりやすく）。
...
`;
    ...
    if (!parsed || (!parsed.html && !parsed.text)) {
      // 失敗した場合は raw をテキストフィールドに入れて返す
      return res.json({ html: "", text: raw });
    }

    res.json({ html: parsed.html || "", text: parsed.text || "" });
  } catch (err) {
